<template>
  <div class="page-shell library-page">
        ...
        <main class="library-content">
          <div class="library-tools">
            <div class="type-tabs" role="tablist" aria-label="资源类型">
              <button v-for="tab in visibleTypeTabs" :key="tab.value" type="button" :class="{ active: activeTab === tab.value }" :disabled="!hasResources && tab.value !== 'all'" @click="activeTab = tab.value">{{ tab.label }}</button>
            </div>
            <span>{{ hasResources ? `${filteredResources.length} 项可用资源` : '等待本次诊断结果' }}</span>
          </div>
    ...
    <el-dialog v-model="previewVisible" width="min(680px, calc(100vw - 32px))" class="resource-preview-dialog" :show-close="true">
      ...
      <p class="dialog-body">{{ selectedPreview?.body }}</p>
...
const typeTabs = computed(() => [{ value: 'all', label: '全部' }, ...resourceTypes.value.map(type => ({ value: type, label: type }))])
const visibleTypeTabs = computed(() => hasResources.value ? typeTabs.value : [{ value: 'all', label: '全部' }, { value: 'lecture', label: '讲义' }, { value: 'practice', label: '实操' }, { value: 'project', label: '项目' }, { value: 'test', label: '复测' }])
const filteredResources = computed(() => { const search = keyword.value.toLowerCase(); return resources.value.filter(item => { const matchesSearch = !search || [item.title, item.knowledge_point,item.body].join(' ').toLowerCase().includes(search); const matchesTab = activeTab.value === 'all' || item.content_type === activeTab.value; const matchesType = !selectedTypes.value.length || selectedTypes.value.includes(item.content_type); const matchesPoint = !selectedPoints.value.length || selectedPoints.value.includes(item.knowledge_point); return matchesSearch && matchesTab && matchesType && matchesPoint }) })
const featuredResource = computed(() => filteredResources.value[0] || null)
const selectedLabel = computed(() => activeTab.value === 'all' ? '基于本次能力缺口' : activeTab.value)
