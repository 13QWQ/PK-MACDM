    ...
    <section class="access-layout">
      ...
      <section class="access-card glass-surface motion-enter motion-delay-1" aria-label="账户访问">
        ...
        <form v-if="activeTab === 'login'" class="access-form" @submit.prevent="handleLogin">
          ...
          <label><span>密码</span><div class="password-field"><input v-model="loginForm.password" :type="showLoginPwd ? 'text' : 'password'" autocomplete="current-password" placeholder="输入密码" @input="loginError = ''" /><button type="button" :aria-label="showLoginPwd ? '隐藏密码' : '显示密码'" @click="showLoginPwd = !showLoginPwd">{{ showLoginPwd ? '隐藏' : '显示' }}</button></div></label>
        ...
        <form v-else class="access-form" @submit.prevent="handleRegister">
          ...
          <label><span>密码</span><div class="password-field"><input v-model="registerForm.password" :type="showRegisterPwd ? 'text' : 'password'" autocomplete="new-password" placeholder="至少 6 位密码" @input="registerError = ''" /><button type="button" :aria-label="showRegisterPwd ? '隐藏密码' : '显示密码'" @click="showRegisterPwd = !showRegisterPwd">{{ showRegisterPwd ? '隐藏' : '显示' }}</button></div></label>
...
function switchTab(tab: 'login' | 'register') { activeTab.value = tab; loginError.value = ''; registerError.value = ''; router.replace({ query: tab === 'register' ? { ...route.query, tab } : Object.fromEntries(Object.entries(route.query).filter(([key]) => key !== 'tab')) }) }
